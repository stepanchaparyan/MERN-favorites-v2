import { defineMessages } from 'react-intl';

export default defineMessages({
  loadingProfile: {
    id: 'profile.loadingProfile',
    defaultMessage: 'Loading profile...',
  },
  noAnyProfile: {
    id: 'profile.noAnyProfile',
    defaultMessage: 'Whoops, the profile is not created for you, yoo can register again and profile will be created for you automatically',
  },
  profilePage: {
    id: 'profile.profilePage',
    defaultMessage: 'Profile page',
  },
  name: {
    id: 'profile.name',
    defaultMessage: 'Name',
  },
  surname: {
    id: 'profile.surname',
    defaultMessage: 'Surname',
  },
  email: {
    id: 'profile.email',
    defaultMessage: 'Email',
  },
  gender: {
    id: 'profile.gender',
    defaultMessage: 'Gender',
  },
  birthday: {
    id: 'profile.birthday',
    defaultMessage: 'BirthDay',
  },
  phone: {
    id: 'profile.phone',
    defaultMessage: 'Phone',
  },
  editCard: {
    id: 'profile.editCard',
    defaultMessage: 'Edit Card',
  },
  goHomePage: {
    id: 'profile.goHomePage',
    defaultMessage: 'Go home page',
  },
});
